...
def build_prompt_template(messages, mode, system_prompt = None, summerize = False):
    ...
    if not summerize:
        ...
        for msg in messages[:-1]:
            prompt.append({"role": msg.get("role"), "content": [{"type": "text", "text":msg.get("text")}] if mode == "vlm" else msg.get("text")})
        if mode == "vlm":
...
def hierarchical_retrieval(collection, embedding_model, chat_docs, query: str, top_n: int = 5, threshold: float = 0.7, user: str = "user"):
    ...
    if chat_docs:
        chat_doc_metadata = {
            doc_id: meta for doc_id, meta in DOCUMENT_METADATA.items()
            if doc_id in chat_docs
        }
    ...
    if relevant_doc_ids:
        chunk_ids = [f"{doc_id}_chunk_{index}" for doc_id, total_chunks in relevant_doc_ids.items() for index in range(total_chunks)]
